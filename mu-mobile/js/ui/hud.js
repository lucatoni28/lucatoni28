...
export function createHud({ bridge, onOpenPanel, resolveIcon, resolveBg }) {
  ...
  const slotNodes = BAR_SLOTS.map((spec, i) => {
    ...
    if (def?.group !== null && def?.group !== undefined) {
      // Icon đã cắt viền nằm cục bộ trong img/, không qua resolver CDN.
  ...
  function update(snap, changed) {
    ...
    if (has('worldIndex')) {
      // Nền radar nằm trong CSS, mà url() trong CSS không đi qua lớp chặn zip
      // được — phải xin blob URL thật. Ở chế độ cdn/flat thì hàm này trả về
      // ngay đường dẫn thường.
