...
export function createBridge({
  ...
  const bridge = {
    ...
    /**
     * Dịch chuyển sang bản đồ khác — đúng sự kiện engine dùng.
     *
     * prepareMap là móc để lớp trên nạp sẵn asset của bản đồ đích TRƯỚC khi
     * engine bắt đầu dựng nó. Ở chế độ zip bước này bắt buộc: engine hỏi đường
     * dẫn đồng bộ, chưa bung xong là nó đi hỏi CDN, mà chơi zip là chơi không
     * mạng. Không có móc thì chuyển map ngay như cũ.
     */
    warp(mapId) {
      if (!prepareMap) {
